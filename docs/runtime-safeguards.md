...
| Upgrade canary | `UPGRADE_CANARY=True` | Once per published commit, Plex server, mode, and compatibility profile | A text/JSON canary report and, after job success, a pass marker in `meta_db.sqlite3` |
...
edits Plex or media output. A failure stops that job and creates:

```text
/config/reports/upgrade-canary-*.txt
/config/reports/upgrade-canary-*.json
```
...
All three report types follow `REPORT_RETENTION`; their text and JSON companions
are retained and removed as one pair. Reports use normalized item identity
